...
import util.solver_parameters as solver_parameters
...
class Algo_iterative():
    ...
    def solve_TR_model(self, Parameters):
        ...
        if self.model.status == 1:
            ...
            self.Stat_probabilities, Transition_rules, Objective = None, None, None
...
class Algo_one_imp():
    ...
    def solve_mTR_model(self, Parameters, best_objective, Transition_rules):
        ...
        self.model = backend.stationer_NU.Setup_TR_Model(Parameters, self.Types, self.J)
        ...
        if Parameters["irreducibility"]:
            backend.stationer_U.Constraint.Add_Irreducibility(self.model, Parameters["nbr_of_classes"], self.Types)
        
        if Parameters["profit"]:
            backend.stationer_U.Constraint.Add_Profit_fixedPremiums(self.model, Parameters["nbr_of_classes"], Parameters["Exp_claims"], Parameters["Ratio_of_types"],self.Types)
        ...
        backend.stationer_U.solve_model(self.model, Parameters["solver"])  
